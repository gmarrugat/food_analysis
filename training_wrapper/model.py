...
from custom_layer import ConcatenateOutputWithSigma, OntologyLayer, OntologyLayerV2
...
class Food_Model(Model_Wrapper):
    ...
    def InceptionResNetV2_Multitask_Ont(self, params):
        ...
        x = OntologyLayer((None,total_concepts), params["ONTOLOGY"])(x)
        #Ont_Layer = OntologyLayer((None,total_concepts), params["ONTOLOGY"])
        #Ont_Layer.build((None,total_concepts))
        ...
        for id_name in sorted_keys:
            ...
            if data['type'] == 'sigma':
                ...
                if data['activation'] == "softmax":
                    ...
                    out_act = Activation(create_relu_advanced(1.0), name=id_name)(out)
                #out_act = Activation(data['activation'], name=id_name)(out)
                outputs_list.append(out_act)
                outputs_matching[id_name] = out_act
    ...
    def InceptionResNetV2_Multitask_Two_Ont(self, params):
        ...
        x = OntologyLayer((None,total_concepts), params["ONTOLOGY"])(x)
        x = OntologyLayer((None,total_concepts), params["ONTOLOGY"], True)(x)
        #Ont_Layer = OntologyLayer((None,total_concepts), params["ONTOLOGY"])
        #Ont_Layer.build((None,total_concepts))
        #Ont_Layer.trainable = False
        #x = Ont_Layer(x)
